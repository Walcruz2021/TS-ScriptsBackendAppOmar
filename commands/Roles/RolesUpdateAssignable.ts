...
import { roleAssignable } from '../../dataset/roleAssignable'
import StorageService from '../../app/Core/Storage/Services/StorageService'

export default class RolesUpdateAssignable extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'roles:update:assignable'
  ...
  private nameFileBackup: string = 'roles-update-assignable.json'
  ...
  private async updateRoleAssignable(role: RolesDocument) {
    try {
      ...
      const updateQuery: Object = { assignable: role.assignable }
  ...
  public async execute(): Promise<void> {
    ...
    for (let index = 0; index < roles.length; index++) {
      ...
      const assignable = role.value ? roleAssignable[role.value] : null
      if (assignable) {
