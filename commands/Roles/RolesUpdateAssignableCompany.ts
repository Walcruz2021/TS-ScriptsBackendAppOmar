...
import { roleAssignableCompany } from '../../dataset/roleAssignableCompany'
import StorageService from '../../app/Core/Storage/Services/StorageService'

export default class RolesUpdateAssignableCompany extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'roles:update:assignable:company'

  /**
   * Command description is displayed in the "help" output
   */
  public static description =
    'Use command to update value in array assignableCompany in Roles'

  private nameFileBackup: string = 'roles-update-assignable-company.json'
  ...
  private async updateRoleAssignable(role: RolesDocument) {
    try {
      ...
      await RolRepository.updateOne(queryFilter, role)
    } catch (err) {
  ...
  public async execute(): Promise<void> {
    ...
    for (let index = 0; index < roles.length; index++) {
      ...
      const assignableCompany = role.value
        ? roleAssignableCompany[role.value]
        : null
      if (assignableCompany) {
        await StorageService.add(role, this.nameFileBackup)
        role.assignableCompany = assignableCompany
  ...
  public async run() {
    this.logger.info(`Init: Update value assignable company`)
    try {
      ...
      this.logger.success(`Successfully: Update value assignable company`)
    } catch (error) {
