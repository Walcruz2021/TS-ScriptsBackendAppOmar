...
export default class UpdateLicenseId extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'update:license:id'

  /**
   * Command description is displayed in the "help" output
   */
  public static description = 'This command update license id'
  private nameFileBackup: string = 'update-license-id.json'
  ...
  public async run() {
    this.logger.success('Started update license id')
    try {
      if (this.rollbackCommand) {
        this.logger.success('Started rollback update license id')
        await this.rollback()
      } else {
        await this.execute()
      }
    ...
    this.logger.success('Update license id Success')
  }
  ...
  public async execute(): Promise<void> {
    ...
    let updateLicenseId = JSON.parse(data.body.toString('utf-8'))

    for (const item of updateLicenseId) {
      let findLicense = await LicenseRepository.findOne({ _id: item._id })

      if (findLicense) {
        let findLicenseById = await LicenseRepository.findOne({
        ...
        if (!findLicenseById) {
          const licenseBackup = {
            ...
            type: 'update',
            previousLicenseId: findLicense.idLicense
          }
          listBackup.push(licenseBackup)

          findLicense.idLicense = item.idLicense
      ...
      i = this.processingProgressBar(i, updateLicenseId.length)
    }
  ...
  public async rollback(): Promise<void> {
    ...
    for (const item of data) {
      await LicenseRepository.updateOne(
        { _id: item.licenseId },
        { idLicense: item.previousLicenseId }
      )
