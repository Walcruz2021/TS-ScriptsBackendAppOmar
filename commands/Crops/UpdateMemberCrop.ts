...
export default class UpdateMemberCrop extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'update:member:crop'

  /**
   * Command description is displayed in the "help" output
   */
  public static description = 'This command update member in crop'

  private nameFileBackup: string = 'update-member-crop.json'
  ...
  public async run() {
    this.logger.success('Started update member in crop')
    try {
      if (this.rollbackCommand) {
        this.logger.success('Started rollback update member in crop')
        await this.rollback()
      } else {
        await this.execute()
      }
    ...
    this.logger.success('Update member in crop Success')
  }
  ...
  public async execute(): Promise<void> {
    ...
    let updateMemberCrop = JSON.parse(data.body.toString('utf-8'))

    for (const item of updateMemberCrop) {
      ...
      if (!findCrop) {
        this.logger.log(
          `Not found Crop with id ${item.cropId} and userId ${item.userId}`
        )
        continue
      }
      ...
      await CropRepository.findOneAndUpdate(
        ...
        {
          $set: {
            ['members.$.type']: item.type,
            ['members.$.identifier']: item.identifier
          }
        }
      )
      ...
      i = this.processingProgressBar(i, updateMemberCrop.length)
    }
