...
export default class AddSignerActivity extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'add:signer:activity'
  /**
   * Command description is displayed in the "help" output
   */
  public static description = 'This command add signer in activity'

  private nameFileBackup: string = 'add-signer-activity.json'
  ...
  public async run() {
    this.logger.success('Started add signer in activity')
    try {
      if (this.rollbackCommand) {
        this.logger.success('Started rollback add signer in activity')
        await this.rollback()
      } else {
        await this.execute()
      }
    ...
    this.logger.success('Add signer in activity Success')
  }
  ...
  public async execute(): Promise<void> {
    ...
    let addSignerActivity = JSON.parse(data.body.toString('utf-8'))

    for (const item of addSignerActivity) {
      ...
      i = this.processingProgressBar(i, addSignerActivity.length)
    }
