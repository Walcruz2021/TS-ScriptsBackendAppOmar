...
export default class UpdateAchievementUnitTypeSupplyBySupplyId extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'update:achievement:unitTypeSupply:by:supplyId'

  /**
   * Command description is displayed in the "help" output
   */
  public static description =
    'This command update achievement unit type supply by supply id'

  private nameFileBackup: string =
    'update-achievement-unitTypeSupply-by-supplyId.json'
  ...
  public async run() {
    this.logger.success(
      'Started update achievement unit type supply by supply id'
    )
    try {
      if (this.rollbackCommand) {
        this.logger.success(
          'Started rollback update achievement unit type supply by supply id'
        )
        await this.rollback()
      } else {
        await this.execute()
    ...
    this.logger.success(
      'Update achievement unit type supply by supply id Success'
    )
  }
  ...
  public async execute(): Promise<void> {
    ...
    for (const data of achievements) {
      ...
      let findSupply: any = await SupplyRepository.findOne({
        _id: data.idSupply
      })

      if (!findAchievement || !findSupply) {
        this.logger.log(
          `Achievement with id ${data.idAchievement} not found or Supply with id ${data.idSupply} not found `
        )
        continue
      }
