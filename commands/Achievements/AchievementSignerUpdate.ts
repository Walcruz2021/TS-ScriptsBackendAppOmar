...
import AchievementRepository from '@ioc:Ucropit/Core/AchievementRepository'
...
import { AchievementDocument } from 'App/Core/Achievement/Infrastructure/Mongoose/Interfaces/Achievement.interface'

export default class AchievementSignerUpdate extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'achievement:signer:update'
  ...
  private nameFileBackup: string = 'achievement-signer-update.json'
  ...
  /**
   * Run command, decide to execute or rollback
   */
  public async run() {
  ...
  public async execute(): Promise<void> {
    ...
    let achievement: AchievementDocument
    const cursor = await AchievementRepository.findByCursor({
    ...
    while ((achievement = await cursor.next())) {
      const signers: Signer[] = achievement.signers.filter(
    ...
    await StorageRepository.create(listAchievementBackup, this.nameFileBackup)
  }
