...
export default class DeleteSupply extends BaseCommand {
  /**
   * Command name is used to run the command
   */
  public static commandName = 'delete:supply'

  /**
   * Command description is displayed in the "help" output
   */
  public static description = 'This command delete supply'

  private nameFileBackup: string = 'delete-supply.json'
  ...
  public async run() {
    this.logger.success('Started delete supply')
    try {
      if (this.rollbackCommand) {
        this.logger.success('Started rollback delete supply')
        await this.rollback()
      } else {
        await this.execute()
      }
    ...
    this.logger.success('Delete supply Success')
  }
  ...
  public async execute(): Promise<void> {
    ...
    let deleteSupply = JSON.parse(data.body.toString('utf-8'))

    for (const item of deleteSupply) {
      ...
      await SupplyRepository.deleteOne({ _id: findSupply._id })
      ...
      i = this.processingProgressBar(i, deleteSupply.length)
    }
  ...
  public async rollback(): Promise<void> {
    ...
    for (const item of data) {
      await SupplyRepository.create(item.data)
